...
public class VehicleService extends BasicService {
    ...
    public VehicleIdList getVehicleIdsForDataset(Dataset dataset) {
        return get("http://api.coxauto-interview.com/api/" + dataset.getDatasetId() + "/vehicles", VehicleIdList.class);
    }

    public List<Vehicle> getAllVehiclesForIds(Dataset dataset, long[] vehicleIds) throws InterruptedException {
        List<Callable<Vehicle>> callables = createCallables(Arrays.stream(vehicleIds).mapToObj(id -> (BasicRequest<Vehicle>) () -> get("http://api.coxauto-interview.com/api/" + dataset.getDatasetId() + "/vehicles/" + id, Vehicle.class)).collect(Collectors.toList()));
