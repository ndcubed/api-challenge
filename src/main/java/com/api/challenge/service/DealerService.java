...
public class DealerService extends BasicService {
    ...
    public List<Dealer> getDealersForIds(Dataset dataset, long[] dealerIds) throws InterruptedException {
        List<Callable<Dealer>> callables = createCallables(Arrays.stream(dealerIds).mapToObj(id -> (BasicRequest<Dealer>) () -> get("http://api.coxauto-interview.com/api/" + dataset.getDatasetId() + "/dealers/" + id, Dealer.class)).collect(Collectors.toList()));
